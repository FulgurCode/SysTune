...
static void on_app_switch_toggled(GtkSwitch *widget, gboolean state, AutostartApp *app) {
    if (state) {
...
static void on_app_chosen(GtkNativeDialog *dialog, gint response, gpointer user_data) {
...
static void on_custom_add_clicked(GtkButton *button, gpointer user_data) {
    ...
    const char *command_text = gtk_editable_get_text(GTK_EDITABLE(data->command_entry));
    const char *desc_text = gtk_editable_get_text(GTK_EDITABLE(data->desc_entry));

    if (command_text && *command_text != '\0') {
        AutostartApp *app = g_new0(AutostartApp, 1);
        app->name = g_strdup(name_text && *name_text != '\0' ? name_text : "Custom Command");
...
static void on_custom_command_clicked(GtkButton *button, gpointer user_data) {
    ...
    g_signal_connect(add_button, "clicked", G_CALLBACK(on_custom_add_clicked), data);
}
...
void autostart_to_stack(GtkStack *stack) {
    ...
    GtkBuilder *builder = gtk_builder_new_from_file("ui/autostart_apps.ui");
    if (!builder) {
        g_printerr("Failed to load autostart_apps.ui\n");
        return;
    }

    AutostartPage = GTK_WIDGET(gtk_builder_get_object(builder, "autostart_apps_page"));
    if (!AutostartPage) {
    ...
    AutostartList = GTK_LIST_BOX(gtk_builder_get_object(builder, "autostart_list"));
    GtkWidget *browse_button = GTK_WIDGET(gtk_builder_get_object(builder, "browse_button"));
    GtkWidget *custom_button = GTK_WIDGET(gtk_builder_get_object(builder, "custom_command_button"));

    g_signal_connect(browse_button, "clicked", G_CALLBACK(on_browse_clicked), NULL);
    g_signal_connect(custom_button, "clicked", G_CALLBACK(on_custom_command_clicked), NULL);
