...
void enable_firewall() { 
    execute_command("pkexec ufw enable"); 
}

void disable_firewall() { 
    execute_command("pkexec ufw disable"); 
}

static void on_ssh_switch_activated(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
static void on_smtp_switch_activated(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
static void on_vnc_switch_activated(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
static void on_firewall_switch_activated(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
static void on_port_switch_activated(GObject *object, GParamSpec *pspec, gpointer user_data) {
...
static void security_to_stack(GtkStack *stack) {
    ...
    AdwSwitchRow *firewall_switch = ADW_SWITCH_ROW(gtk_builder_get_object(security_builder, "firewall_switch"));
    g_signal_connect(firewall_switch, "notify::active", G_CALLBACK(on_firewall_switch_activated), NULL);

    GtkSwitch *ssh_switch = GTK_SWITCH(gtk_builder_get_object(security_builder, "ssh_switch"));
    g_signal_connect(ssh_switch, "notify::active", G_CALLBACK(on_ssh_switch_activated), NULL);

    GtkSwitch *vnc_switch = GTK_SWITCH(gtk_builder_get_object(security_builder, "vnc_switch"));
    g_signal_connect(vnc_switch, "notify::active", G_CALLBACK(on_vnc_switch_activated), NULL);

    GtkSwitch *smtp_switch = GTK_SWITCH(gtk_builder_get_object(security_builder, "smtp_switch"));
    g_signal_connect(smtp_switch, "notify::active", G_CALLBACK(on_smtp_switch_activated), NULL);

    GtkEntry *port_entry = GTK_ENTRY(gtk_builder_get_object(security_builder, "port_entry"));
    GtkSwitch *port_switch = GTK_SWITCH(gtk_builder_get_object(security_builder, "port_switch"));
    g_signal_connect(port_switch, "notify::active", G_CALLBACK(on_port_switch_activated), port_entry);
